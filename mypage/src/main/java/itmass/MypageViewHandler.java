...
public class MypageViewHandler {
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenSignedIn_then_CREATE_2 (@Payload SignedIn signedIn) {
        try {
            if (signedIn.isMe()) {
                // view 객체 생성
                Mypage mypage = new Mypage();
                // view 객체에 이벤트의 Value 를 set 함
                mypage.setMemId(signedIn.getMemId());
                mypage.setPhoneNum(signedIn.getPhoneNum());
                mypage.setAddr(signedIn.getAddr());
                // view 레파지 토리에 save
                mypageRepository.save(mypage);
            }
        }catch (Exception e){
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenMemChanged_then_UPDATE_3(@Payload MemChanged memChanged) {
        try {
            if (memChanged.isMe()) {
                // view 객체 조회
                List<Mypage> mypageList = mypageRepository.findByMemId(memChanged.getMemId());
                for(Mypage mypage : mypageList){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
                    mypage.setPhoneNum(memChanged.getPhoneNum());
                    mypage.setAddr(memChanged.getAddr());
                    // view 레파지 토리에 save
                    mypageRepository.save(mypage);
                }
            }
